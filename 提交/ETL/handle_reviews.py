...
'''
def addReviewNum():
    movie_info = pd.read_csv('./movie_info.csv', encoding='utf-8')

    movie_review_num = pd.read_csv('./movie_review_num.csv', encoding='utf-8')

    movie_review_num.drop_duplicates(subset=['movie_id'], keep='first', inplace=True)

    output = pd.merge(movie_info, movie_review_num, on='movie_id', how='left')

    output.to_csv('./movie_info.csv', index=False, encoding='utf-8')
