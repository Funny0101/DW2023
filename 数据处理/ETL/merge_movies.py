...
def merge_movies():
    # 读取CSV文件
    df = pd.read_csv('filtered_movie.csv')

    # 1.删除大概率错误数据
    # 删除 'Title' 列为空的行
    output = output.dropna(subset=['movie_title'])

    # 2.去重并记录数据血缘，认为电影名和导演都相同的为同一部电影
    ...
    result_of[non_movie_id_columns] = np.where(mask, pd.NA, result_of[non_movie_id_columns])
    # 去除末尾的movie_id
    ...
    # 保存映射关系
    result_of.to_csv('result_of.csv', index=False)
